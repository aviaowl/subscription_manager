...
def test_add_subscription_with_none_fields(controller, subscription_dict, paramNone):
    ...
    expected_exc_msg = f"Found wrong field '{paramNone}' type, " \
                       f"expected:{type(subscription_dict[paramNone])}, received:(<class ""'NoneType'>, None)"
...
@pytest.mark.parametrize("field, value",
                         [('owner', 123),
                          ...
                          ('currency', 1.01),
                          ('comment', 42)])
def test_add_subscription_wrong_types(subscription_dict, controller, field, value):
    ...
    with pytest.raises(SubscriptionException) as exc:
        assert controller.add_subscription(sub_dict)
    # Receive expected type from valid subscription instance by field name
    expected_exc_msg = f"Found wrong field '{field}' type, " \
